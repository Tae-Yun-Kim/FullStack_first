...
import com.back4.dto.payment.PaymentHistoryDTO;
import com.back4.dto.payment.PaymentRequestDTO;
import com.back4.dto.payment.PaymentResponseDTO;
import com.back4.repository.member.MemberRepository;
import com.back4.repository.payment.PaymentHistoryRepository;
import com.back4.repository.payment.PaymentRepository;
...
//    }
    public void processPayment(PaymentRequestDTO paymentRequest) {
        // 예: Member 조회
        Member member = memberRepository.findByEmail(paymentRequest.getMemberEmail())
    ...
    @Transactional
    public void requestCancelPayment(String impUid, String email) {
        Payment payment = paymentRepository.findByImpUidAndMemberEmail(impUid, email)
                .orElseThrow(() -> new IllegalArgumentException("결제를 찾을 수 없습니다."));
    ...
    public List<PaymentHistoryDTO> getUserCancelRequests(String email) {
        return paymentRepository.findByMemberEmailAndStatus(email, PaymentStatus.REFUND_REQUESTED)
                .stream()
                .map(payment -> new PaymentHistoryDTO(
                        payment.getImpUid(),
                        payment.getMerchantUid(),
